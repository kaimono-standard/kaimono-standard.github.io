...
description: 買いもの標準（このリポジトリ）に楽天市場・Amazon の商品リンク付き比較記事を1本追加するための手順。記事の型は「○○5機種を比較」（既定）と「新型Aと旧型Bの違い」（新旧比較）の2つ。ユーザーが「記事を増やしたい」「○○の比較記事を書いて」「新旧比較」「型落ちとの違い」「新しいカテゴリを追加」「ケトル記事と同じ形式で」と言ったとき、商品ジャンル名や型番だけを投げてきたとき、既存の比較記事の製品差し替え・更新を頼まれたときに必ず使う。メーカー公式仕様の裏取り → 商品リンク取得 → Codex への下書き委任 → 事実照合 → 承認キューへの提出までを、事実ベースで完走する。
---
...
| 型 | `article_type` | 製品数 | 使う場面 | 完成例・ルール |
|---|---|---|---|---|
